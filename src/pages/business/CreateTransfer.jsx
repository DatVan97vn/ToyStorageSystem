...
import { getInventoryByWarehouse } from "../../api/business/inventoryApi";
...
function CreateTransfer() {
  ...
  return (
  <div className="page-container transfer-create-page">
    <div className="transfer-create-header">
      <div>
        <h1>Tạo phiếu điều kho</h1>
        <p>
          Tạo phiếu chuyển hàng từ kho xuất sang kho nhập và kiểm soát số lượng
          theo tồn kho thực tế.
        </p>
      </div>
    </div>
    ...
    <form onSubmit={handleSubmit}>
      <div className="transfer-form-card">
        ...
        {!fromWarehouseId && (
          <p className="transfer-warning">
            Vui lòng chọn kho xuất để tải tồn kho.
          </p>
        )}

        {fromWarehouseId && inventory.length === 0 && (
          <p className="transfer-warning">
            Kho xuất hiện chưa có sản phẩm tồn kho.
          </p>
        )}
      </div>
            ...
            >
              ...
              {availableProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    {product.sku} - {product.name} - Tồn còn lại:{" "}
                    {remainingQty}
                  </option>
                );
              })}
      ...
      <div className="transfer-form-card">
        ...
        <div className="transfer-table-scroll">
          <table className="transfer-items-table">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr>
                  <td colSpan="8" className="transfer-empty">
                ...
                items.map((item, index) => (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td className="code-cell">{item.sku}</td>
                    <td>{item.barcode || "-"}</td>
        ...
        <div className="transfer-submit-row">
